<template>
    <div>
        <div class="form-group">
            <label>
                {{ form.label }}
                <span v-if="form.required" class="text-danger">*</span>
            </label>
            <div class="custom-file">
                <input
                    type="file"
                    class="custom-file-input"
                    :name="form.name"
                    :id="formatId"
                    :accept="accept"
                    :disabled="form.disabled"
                    v-on:change="onChange"
                />
                <label class="custom-file-label" :for="formatId">{{
                    labelInput
                }}</label>
            </div>
        </div>
        <div id="previewImageDiv" class="row hide">
            <div class="col text-center">
                <img
                    src="data:image/jpeg;base64,"
                    alt=""
                    id="previewImage"
                />
            </div>
        </div>
        <div v-if="previewEdit" id="" class="row">
            <div class="col text-center">
                <a :href="fileEditUrl" target="__blank">
                    <span v-if="form.format === 'DOCUMENT'">
                        Ver Archivo: {{ labelInput }}
                    </span>
                    <img
                        v-if="form.format === 'DOCUMENT[IMAGE]'"
                        :src="fileEditUrl"
                        alt=""
                        id="previewImageEdit"
                    />
                </a>
            </div>
        </div>
    </div>
</template>

<script>
/**
 * @author Jorge Peraza
 * @version 0.0.1
 */
import refactor from "../mixins/mix";
export default {
    mixins: [refactor],
    name: "sheet-input-document",
    data: () => ({
        fileList: [],
        imgSrc: "",
        labelInput: "Seleccione un archivo",
        storage_url: process.env.MIX_SHEETS_STORAGE_URL,
        previewEdit: false,
        fileEditUrl: ""
    }),
    computed: {
        formatId() {
            return `${this.form.id}`;
        },
        files() {
            return this.fileList.join(" ");
        }
    },
    mounted() {
        this.checkFileEdit();
    },
    methods: {
        checkFileEdit() {
            const isEdit = this.$store.getters["form/recordid"];
            if (isEdit !== null) {
                const entities_fk = this.$store.getters["form/entities_fk"];
                const fieldsvalues = this.$store.getters["form/fieldsvalues"];
                const fileId = fieldsvalues[this.id];
                this.previewEdit = true;
                Object.keys(entities_fk).forEach(key => {
                    entities_fk[key].map(entity => {
                        if (entity.id === fileId) {
                            console.log('entró', entity);
                            this.labelInput = entity.name;
                            this.fileEditUrl = `${this.storage_url}${entity.src}`;
                        }
                    });
                });
            } else {
                console.log("no es edit");
            }
        },
        onChange(event) {
            this.previewEdit = false;
            const input = event.target;
            document.getElementById("previewImageDiv").classList.remove("show");
            document.getElementById("previewImageDiv").classList.add("hide");
            this.labelInput = input.files[0].name;
            if (this.form.format === "DOCUMENT[IMAGE]") {
                document.getElementById(
                    "previewImage"
                ).src = URL.createObjectURL(event.target.files[0]);
                document
                    .getElementById("previewImageDiv")
                    .classList.remove("hide");
                document
                    .getElementById("previewImageDiv")
                    .classList.add("show");
            }
            this.$emit("sheets-file-change", event, this.form.id);
        }
    }
};
</script>

<style lang="scss">
.file {
    opacity: 0;
    width: 0.1px;
    height: 0.1px;
    position: absolute;
}
.file-input {
    label {
        display: block;
        position: relative;
        // width: 150px;
        height: 30px;
        border-radius: 5px;
        background: #aaa8a9;
        // margin: 3px;
        /* linear-gradient(40deg, , #0f607996); */
        box-shadow: 0 2px 3px rgba(0, 0, 0, 0.4);
        display: flex;
        align-items: center;
        justify-content: center;
        color: #fff;
        font-weight: bold;
        cursor: pointer;
        transition: transform 0.2s ease-out;
        i {
            margin: 5px;
        }
    }
}

input:focus + label {
    outline: 1px solid #000;
    outline: -webkit-focus-ring-color auto 2px;
}
.show {
    display: block;
}
.hide {
    display: none;
}
#previewImage {
    max-width: 50%;
    border-radius: 25px;
}
#previewImageEdit {
    max-width: 50%;
    border-radius: 25px;
}
</style>
